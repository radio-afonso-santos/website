...
function keyboardShortcut(options, closure) {
  ...
  if (
    options.selector.which == keyboardKeys[options.key] ||
    options.selector.keyCode == keyboardKeys[options.key]
  ) {
...
$(document).ready(function() {
  $(document).keydown(function(e) {
    if (e.which == keyboardKeys[config.keys.active]) {
      toggleKey('active');
    }
    if (e.which == keyboardKeys[config.keys.debug]) {
      toggleKey('debug');
    }
    /*
         on key down verify special keys
         */
    if (e.which == keyboardKeys.shift) {
      config.press.shift = true;
      return true;
    }
    if (e.which == keyboardKeys.ctrl) {
      config.press.ctrl = true;
      return true;
    }
    if (e.which == keyboardKeys.alt) {
      config.press.alt = true;
      return true;
    }
  });
  /*
